...
fn request(
    ...
    request_with_timeout(method, url, headers, body, TIMEOUT_MS, CONNECT_TIMEOUT_MS)
}

/// Perform one plain HTTPS GET on behalf of an application route.
///
/// Reuses the hardened libcurl path above — certificate verification always on,
/// redirects never followed, `NOSIGNAL` set for thread safety — without Stripe's
/// auth header, retry loop, or circuit breaker, which are specific to the
/// payments API. `timeout_ms` bounds the whole transfer; the connect phase gets
/// the same budget, capped at the Stripe connect timeout.
///
/// # Errors
/// Returns [`StripeError::Transport`] when libcurl cannot complete the transfer.
pub fn https_get(
    url: &str,
    headers: &[(&str, &str)],
    timeout_ms: i64,
) -> Result<(u16, Vec<u8>), StripeError> {
    let total = timeout_ms.clamp(1, c_long::MAX as i64) as c_long;
    let res = request_with_timeout(Method::Get, url, headers, None, total, total.min(CONNECT_TIMEOUT_MS))?;
