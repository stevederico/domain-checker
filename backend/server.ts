...
/**
 * Validate a parsed config.json value matches the raw config shape.
 *
 * Narrows unknown JSON before it is trusted as the raw configuration: checks
 * the `database` block carries the string fields server.ts reads.
 *
 * @param value - Parsed JSON value from config.json
 * @returns True if the value is a valid raw config
 */
