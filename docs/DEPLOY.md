...
- **Set `NODE_ENV=production`** in all deployment environments. Without this, error responses may include stack traces and internal details. The included Dockerfile already sets this.
- **Place behind a reverse proxy** (nginx, Cloudflare, etc.) that sets `x-forwarded-for` for accurate rate limiting on the domain check endpoint (30 req/min per IP).
